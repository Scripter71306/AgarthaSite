...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!submitted) return { statusCode: 400, body: JSON.stringify({ ok:false }) };
    ...
    const resp = await fetch(url, { headers: { Accept: 'application/vnd.github.v3.raw', Authorization: `token ${TOKEN}` }});
    if (!resp.ok) return { statusCode: 500, body: JSON.stringify({ ok:false }) };
